...
""")
        self._html_handle.write(
            f"<header><h1>{html.escape(title)}</h1><div class=\"meta\">Started: {html.escape(now)}</div></header>\n")
    ...
    def _write_html_event(self, event_obj: Dict[str, Any]):
        ...
        if event == "user_input":
            ...
            if role == "user":
                ...
                lines.append(
                    f"<div class=\"block\"><h3>🔧 Message Written: Tool ({self._escape_html(str(tool_name))})</h3>")
            ...
            if usage:
                lines.append(
                    f"<div class=\"meta\">Tokens: {usage.get('prompt_tokens', 0)} → {usage.get('completion_tokens', 0)} = {usage.get('total_tokens', 0)}</div>")
            if tool_calls:
